...
class GroupCreateView(PermissionMixin, CreateViewMixin, CreateView):
    ...
    form_class = GroupForm  # ← Corrección
    success_url = reverse_lazy('security:group_list')
    permission_required = 'add_group'

    def get_context_data(self, **kwargs):
        context = super().get_context_data()
        context['grabar'] = 'Grabar Group'
    ...
    def form_valid(self, form):
        ...
        group = self.object
        messages.success(self.request, f"Éxito al crear el Group {group.name}.")
        return response
...
class GroupUpdateView(PermissionMixin, UpdateViewMixin, UpdateView):
    ...
    success_url = reverse_lazy('security:group_list')  # ← Corrección
    permission_required = 'change_group'

    def get_context_data(self, **kwargs):
        context = super().get_context_data()
        context['grabar'] = 'Actualizar Group'
        context['back_url'] = self.success_url
        return context
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        group = self.object
        messages.success(self.request, f"Éxito al actualizar el Group {group.name}.")
        return response
...
class GroupDeleteView(PermissionMixin, DeleteViewMixin, DeleteView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data()
        context['grabar'] = 'Eliminar Group'
        context['description'] = f"¿Desea eliminar el Group: {self.object.name}?"
        context['back_url'] = self.success_url
        return context
    ...
    def form_valid(self, form):
        ...
        response = super().form_valid(form)
        messages.success(self.request, f"Éxito al eliminar lógicamente el Group {group_name}.")
        return response
